...
import { FRAME_SHAPES, frameShapeLabel, type FrameShape } from '../../data/visagismo';
...
const EMPTY_FORM: FormState = {
  ...
  category: '', frame_shape: '', description: '', imageFile: null, image_url: '',
};
...
const AdminProducts = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Solo se envía si hay valor, para no romper el guardado si aún no se corrió la migración
      if (formData.frame_shape) {
        payload.frame_shape = formData.frame_shape as FrameShape;
      } else if (mode === 'edit' && editingId && products.find(p => p.id === editingId)?.frame_shape) {
        payload.frame_shape = null;
      }
      ...
      {mode !== 'none' && (
        <div className="bg-white p-8 rounded-2xl shadow-lg mb-10 border-l-4 border-primary-gold">
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Forma de montura</label>
              <select
              ...
              <p className="text-xs text-gray-400 mt-1">
                Se usa para recomendar esta montura en el visagismo.
              </p>
            </div>
                  ...
                  >
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-sm text-gray-400">
                        {product.brand}
                        {product.frame_shape && ` · ${frameShapeLabel(product.frame_shape)}`}
                      </div>
                    </td>
